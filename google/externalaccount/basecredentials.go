...
The [authelia.com/client/oauth2.TokenSource] created from the config using [NewTokenSource] can then be used to access Google
...
The [authelia.com/client/oauth2.TokenSource] created from the config using [NewTokenSource] can then be used access Google
Cloud resources. For instance, you can create a new client from the
[cloud.google.com/go/storage] package and pass in option.WithTokenSource(yourTokenSource))
