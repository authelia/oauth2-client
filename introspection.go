...
func (c *Config) IntrospectToken(ctx context.Context, token *Token, opts ...IntrospectionRevocationOption) (results []IntrospectionResult, err error) {
	if token == nil {
		return nil, fmt.Errorf("error revoking token: no token was provided")
	}

	if c.Endpoint.RevocationURL == "" {
		return nil, fmt.Errorf("error revoking token: no revocation endpoint URL was provided")
	}
	...
	for i, tth := range tths {
		...
		case "access_token":
			if len(token.AccessToken) == 0 {
				return nil, fmt.Errorf("error introspecting token: token type hint '%s' can only be revoked for a token that has an access token", tth)
			}

			xvals.Set("token", token.AccessToken)
		case "refresh_token":
			if len(token.AccessToken) == 0 {
				return nil, fmt.Errorf("error introspecting token: token type hint '%s' can only be revoked for a token that has a refresh token", tth)
			}
	...
	for _, v := range vals {
		if introspection, err = internal.IntrospectToken(ctx, c.ClientID, c.ClientSecret, c.Endpoint.RevocationURL, v, internal.AuthStyle(c.Endpoint.AuthStyle), c.authStyleCache.Get()); err != nil {
			errored = true
